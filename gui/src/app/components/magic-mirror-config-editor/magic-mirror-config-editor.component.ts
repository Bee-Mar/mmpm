...
import { MonacoEditorComponent, MonacoEditorConstructionOptions, MonacoStandaloneCodeEditor, MonacoEditorLoaderService } from "@materia-ui/ngx-monaco-editor";
...
export class MagicMirrorConfigEditorComponent implements OnInit {
  ...
  monacoComponent: MonacoEditorComponent;
    ...
    private readonly _mmpmEditorThemeCookie: string = "MMPM-editor-theme";
    private readonly _mmpmEditorCurrentFileCookie: string = "MMPM-editor-current-file";
    ...
    public ngOnInit(): void {
        this.editorOptions = {
            language: "javascript",
            theme: Cookie.get(this._mmpmEditorThemeCookie) ?? "vs-dark",
    ...
    public onToggleTheme() {
        ...
        Cookie.set(this._mmpmEditorThemeCookie, newTheme, {
            expires: 1825,
            path: "",
        });
    }
    ...
    public setFileIndexAndCookie(index: number = 0): void {
        ...
        monaco.editor.setModelLanguage(this.editor.getModel(), this.fileSelection[this.fileIndex].syntax);
        Cookie.set(this._mmpmEditorCurrentFileCookie, String(index), {
            expires: 1825,
            path: "",
        });
    }
    ...
    private _initFileIndex(): void {
        this.fileIndex = Number(Cookie.get(this._mmpmEditorCurrentFileCookie)) ?? ConfigFile.CONFIG_JS;
    }
}
